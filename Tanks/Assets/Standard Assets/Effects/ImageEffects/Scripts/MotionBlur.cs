...
namespace UnityStandardAssets.ImageEffects {
  ...
  public class MotionBlur : ImageEffectBase {
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination) {
      ...
      // If Extra Blur's selected, downscale texture to 4x4 smaller resolution
      if (extraBlur) {
        RenderTexture blurbuffer = RenderTexture.GetTemporary(source.width / 4,
          source.height / 4, 0);
      ...
      // Clamp the motion blur variable so it can never leave permanent trails
